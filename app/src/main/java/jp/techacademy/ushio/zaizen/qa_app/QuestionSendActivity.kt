package jp.techacademy.ushio.zaizen.qa_app
...
class QuestionSendActivity : AppCompatActivity(), View.OnClickListener, DatabaseReference.CompletionListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.d("uztest","onCreate end")
    }

    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        super.onActivityResult(requestCode, resultCode, data)
        Log.d("uztest","onActivityResult Start")
        ...
        Log.d("uztest","onActivityResult end")

    }

    override fun onClick(v: View) {
        Log.d("uztest","onClick Start")

        if (v === imageView) {
            Log.d("uztest","biuldcheck 1")
            // パーミッションの許可状態を確認する
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                Log.d("uztest","buildcheck 2")
                if (checkSelfPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
                    // 許可されている
                    Log.d("uztest","permissiongranted")
                    showChooser()
                } else {
                    // 許可されていないので許可ダイアログを表示する
                        Log.d("uztest","permission denied")
            ...
            Log.d("uztest",data.toString())
            progressBar.visibility = View.VISIBLE
        }
        Log.d("uztest","onClick end")

    }

    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<String>, grantResults: IntArray) {
        Log.d("uztest","onRequestPermissionResult Start")
        ...
        Log.d("uztest","onRequestPermissionResult end")

    }

    private fun showChooser() {
        Log.d("uztest","showChoose Start")
        ...
        Log.d("uztest","showChoose end")
    }

    override fun onComplete(databaseError: DatabaseError?, databaseReference: DatabaseReference) {
        progressBar.visibility = View.GONE

        Log.d("uztest","onComplete Start")
