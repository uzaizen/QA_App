...
class LoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mCreateAccountListener = OnCompleteListener { task ->
            if (task.isSuccessful) {
                // 成功した場合
                    Log.d("uztest", "createaccount successful")
                // ログインを行う
                val email = emailText.text.toString()
                val password = passwordText.text.toString()
                login(email, password)
                Log.d("uztest","Login Done")
            } else {

                // 失敗した場合
                    Log.d("uztest","createaccount failed")
                // エラーを表示する
        ...
        createButton.setOnClickListener { v ->
            ...
            if (email.length != 0 && password.length >= 6 && name.length != 0) {
                ...
                Log.d("uztest","createaccount done")
            } else {
                // エラーを表示する
    ...
    private fun createAccount(email: String, password: String) {
        ...
        // アカウントを作成する
        Log.d("uztest","createuser1")
        mAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener(mCreateAccountListener)
        Log.d("uztest","createuser2")
    }
